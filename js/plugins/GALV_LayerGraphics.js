 ...
 *
 * @param Clear Layer Cache
 * @desc true or false - A trial option to clear layers from cache if memory issues are experienced
 * @default false
 *
 * @param Tile Size
...
Galv.LG.clearCache = PluginManager.parameters('Galv_LayerGraphics')["Clear Layer Cache"].toLowerCase() == "true" ? true : false;
...
Spriteset_Map.prototype.createLayerGraphics = function() {
	...
	for (var propertyId in mapGraphics) {
		...
		if (!this.layerGraphics.hasOwnProperty(propertyId)) {
			// Create Layer Sprite
			if ($gameMap.layerConfig()[propertyId]) {
				this.layerGraphics[propertyId] = new Sprite_LayerGraphic(propertyId);
...
var Galv_Scene_Map_terminate = Scene_Map.prototype.terminate;
Scene_Map.prototype.terminate = function() {
	Galv_Scene_Map_terminate.call(this);
	this._spriteset.destroyLayers();
};

Spriteset_Map.prototype.destroyLayers = function() {
	for (var sprite in this.layerGraphics) {
		if (Galv.LG.clearCache) this.layerGraphics[sprite]._destroyCachedSprite();
		this.layerGraphics[sprite] = null;
	};
};
