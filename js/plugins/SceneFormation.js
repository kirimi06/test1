...
Imported['SceneFormation'] = 1.05;

/*:
 * @plugindesc ver1.05/並び替えシーンを追加します。
 * @author Yana
 *
 * 
...
(function() {
	...
	Window_FormationStatus.prototype.drawParameters = function(x, y) {
		...
		for (var i = 0; i < 6; i++) {
			var paramId = i + 2;
			var y2 = y + 32 * (i % 3);
			var x2 = x + Math.floor(i / 3) * 196;
			this.changeTextColor(this.systemColor());
			this.drawText(TextManager.param(paramId), x2, y2, 120);
			this.resetTextColor();
			this.drawText(this._actor.param(paramId), x2 + 120, y2, 60, 'right');
		}
	};
	...
	Window_FormationStatus.prototype.drawExpInfo = function(x, y) {
		...
		this.changeTextColor(this.systemColor());
		this.drawText(expTotal, x, y + lineHeight * 0, 372);
		this.drawText(expNext, x, y + lineHeight * 1, 372);
		this.resetTextColor();
		this.drawText(value1, x, y + lineHeight * 0, 372, 'right');
		this.drawText(value2, x, y + lineHeight * 1, 372, 'right');
	};
